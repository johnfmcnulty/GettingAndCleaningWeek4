## Begin by reading in all relevant data
...
features_mod <- lapply(features$V2, tolower)
features_mod <- sub("bodybody", ".body", features_mod)
features_mod <- sub("tbody.", "time.body", features_mod)
features_mod <- sub("tgravity", "time.gravity", features_mod)
features_mod <- sub("f.", "frequency.", features_mod)
features_mod <- sub(".ody", ".body", features_mod)
features_mod <- sub(".ravity", ".gravity", features_mod)
features_mod <- sub("acc", ".acc", features_mod)
features_mod <- sub("gyro", ".gyro", features_mod)
features_mod <- sub("mag", ".mag", features_mod)
features_mod <- gsub("-", ".", features_mod)
features_mod <- gsub(",", ".", features_mod)

## Binding together column headers, test and traing sets.
col_header <- c(features_mod, "subject","activity")
empty_data <- as.data.frame(matrix(nrow = 0, ncol = 563))
bind_data <- bind_rows(train, test)
master_data <- bind_rows(bind_data, empty_data)
colnames(master_data) <- col_header
...
## Finally, some basic housekeeping.  Removing objects that are no longer needed
rm(features)
rm(features_mod)
rm(col_header)
rm(empty_data)
